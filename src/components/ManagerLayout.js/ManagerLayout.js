...
function ManagerLayoutContent({ children }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Permitir manager ou admin impersonating
      if (parsedUser.role !== "manager" && !Cookies.get("admin_impersonating")) {
        router.push("/login");
      }
    } catch (error) {
      router.push("/login");
  ...
  return (
    <div className="relative min-h-screen w-full bg-background overflow-hidden flex">
      ...
      {/* --- SIDEBAR (MODO MANAGER) --- */}
      <AppSidebar mode="manager" isCollapsed={isCollapsed} onCollapse={setIsCollapsed} />
        ...
        <header className="h-16 mb-6 glass rounded-2xl px-4 sm:px-6 flex items-center justify-end sm:justify-between sticky top-4 z-30 bg-white/40 dark:bg-black/20 backdrop-blur-xl border border-white/20 dark:border-white/5 shadow-sm ml-12 lg:ml-0">
          ...
          <div className="hidden sm:flex items-center gap-4 w-full max-w-md">
            <div className="relative w-full group">
              ...
              <input 
                type="text" 
                placeholder="Pesquisar en el Restaurante..." 
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <div className="text-xs font-black text-muted-foreground hidden sm:block">
               RESTAURANT PANEL v2.0
            </div>
          </div>
        </header>
